...
class Arguments:
    ...
    name: str = field(default="may1-20seeds-5041479ec9f78561c124d99741842553")
    model_dir: str = field(default="/storage/quach/snapshots/repg2/image-sentence")
...
def main():
    ...
    scores_filename = (os.path.join(args.base, "image-sentence_{}_{}".format(args.start_seed, args.end_seed), "image-sentence-raw-{}.pt".format(args.name)))
    gold_filename = (os.path.join(args.base, "image-sentence_{}_{}".format(args.start_seed, args.end_seed), "image-sentence-gold-{}.pt".format(args.name)))

    os.makedirs(os.path.dirname(scores_filename), exist_ok=True)

    torch.save(scores, scores_filename)
    torch.save(gold, gold_filename)

    print("Saved to", scores_filename)
